...
def tzfunctional(request):
    return render(request,'pytzexample.html')
def timedisplay(request):
    return render(request,'timedisplay.html')
def weatherlogic(request):
    if request.method == 'POST':
        place = request.POST['place']
        API_KEY = '6bb3c81cd13d14428490b99c6f0b593e'
        ...
        if response.status_code == 200:
            ...
            return render(request, 'timedisplay.html', {'error_message': error_message})
...
def images(request):
    return render(request,'touristplaces.html')
...
def contactmail(request):
    if request.method=="POST":
        ...
        tosend=comment + '--------------------This is just the commentbox'
        data=contactus(firstname=firstname,lastname=lastname,email=email,comments=comment)
        data.save()
        return HttpResponse("<h1><center>Thankyou for giving feedback </center></h1>")
